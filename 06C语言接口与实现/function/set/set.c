...
#define T   Set_T
/*
 * 使用哈希表表示接合并使用比较函数 和 哈希函数在表中定位成员
 */
struct T{
    int length;//记录接合中成员中的数目
    unsigned timestamp;//用于实现Set_map中的已检查运行错误 即禁止apply修改
                        //集合
...
static int cmpaton(const void *x, const void *y)
{
    return x!=y;
}
            ...
            unsigned hash(const void *x));
{
    ...
    assert(h >= 0);
    for(i =1 ; primes[i] < h); i++
        ;
    set = ALLOC(sizeof(*set) + primes[i-1]sizeof(set->buckets[0]));
    set ->size = primes[i-1];
    set ->cmp = cmp ? cmp : cmpatiom;
    set -> hash = hash ? hash : hashatiom;
...
void Set_map(T set, void apply(const void *member,void *cl),
                void *cl)
{
    int i;
    unsigned stamp;
    ...
    stamp = set->timestamp;
    for(i=0 ; i < set->size; i++)
        for(p = set->buckets[i]; p; p->link){
            apply(p->member,cl);
            assert(set->timestamp == stamp);//不让map函数调用接口函数修
                                            //改集合
        }
}
...
T Set_inter(T s, T t) {
	if (s == NULL) {
		...
		return Set_inter(t, s);//其实可以直接调换 s和t 不必要递归
	else {
