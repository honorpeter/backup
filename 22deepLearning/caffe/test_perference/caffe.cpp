...
void collect(){
    collect_count_ = 0;
    int count = 0;
    cudaError_t cuda_status = cudaGetDeviceCount(&count);
    if(cuda_status != cudaSuccess){
        return;
    }

    per_gpu_memory_ = std::vector<float>(count);

    while(1){
        //先等待60秒 初始化内存不稳定
        boost::this_thread::sleep(boost::posix_time::seconds(1));
        get_memory(cpu_memory_);
        get_gpu_memory(per_gpu_memory_);
...
//hhh
	boost::timer::cpu_timer moudle_timer;
  	boost::thread collect_memory_ = boost::thread(collect);
  ...
  if (gpus.size() > 1) {
	...
	moudle_timer.stop();
   	std::cout<<"RUNNING_TIME"<<
             moudle_timer.format(3, "%w")<<std::endl;
    	std::cout<<"USER_CPU_TIME"<<
             moudle_timer.format(3, "%u")<<std::endl;
    	std::cout<<"SYSTEM_CPU_TIME"<<
             moudle_timer.format(3, "%s")<<std::endl;
    collect_memory_.interrupt();
    collect_memory_.join();
    std::cout<<"USE_MEMORY "<<(cpu_memory_ / collect_count_ )<< " MB"<<std::endl;
    std::string gpu_messages;
    int size = per_gpu_memory_.size();
    for (int i = 0; i < size; ++i) {
        std::cout<< "gpu: "<<i<<" 使用显存为 "
                         <<(per_gpu_memory_[i]/collect_count_)<<
                         " MB +\n"<<std::endl;
    }
